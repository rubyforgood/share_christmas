...
RSpec.describe Orgadmin::MembershipsController, type: :controller do
  ...
  describe 'create >' do
    ...
    it 'creates a new user, if a user with that email doesn\'t exist' do
      expect do
        post :create, user: willy_smith_attr
    ...
    it 'creates a membership, if it doesn\'t exist' do
      ...
      expect(response).to redirect_to orgadmin_memberships_path
    end
    ...
    it 'matches user if recipient is given' do
      ...
      post :create, user: willy_smith_attr, recipient_id: recip.id
      recip.reload
  ...
  describe 'update >' do
    ...
    it 'does not allow an edit to produce a duplicate email' do
      ...
      willy_smith_attr[:password] = willy_smith_attr[:password_confirmation] = 'BLUGGGGH'
      User.create!(willy_smith_user_attr)
      patch :update, id: m.id, user: willy_smith_attr
